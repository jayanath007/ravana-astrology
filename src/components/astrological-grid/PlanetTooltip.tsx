...
export function PlanetTooltip({
  ...
  // Check if we have sign change dates available
  const hasSignChangeData =
    planetData?.nextSignChangeDate || planetData?.lastSignChangeDate;
        ...
        createPortal(
          ...
          >
            <div className="space-y-2">
              {/* Show sign change dates if available, otherwise show nakshatra details */}
              {hasSignChangeData ? (
                <>
                  {planetData?.nextSignChangeDate && (
                    <div className="flex items-start gap-3">
                      ...
                      <span className="text-gray-900 dark:text-gray-100 text-[22px]">
                        {formatDateString(planetData.nextSignChangeDate)}
                      </span>
                    </div>
                    
                  )}
                  {planetData?.lastSignChangeDate && (
                    <div className="flex items-start gap-3">
                      <span className="font-semibold text-gray-700 dark:text-gray-300 min-w-[180px] text-[22px]">
                        අවසන් ග්‍රහ මාරුවූ දිනය: 
                      </span>
                      <span className="text-gray-900 dark:text-gray-100 text-[22px]">
                        {formatDateString(planetData.lastSignChangeDate)}
                      </span>
                    </div>
                  )}
                </>
